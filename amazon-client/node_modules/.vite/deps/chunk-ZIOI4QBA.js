...
var Scope = class {
   ...
   */
  addDeclaration(identifier2, context, init, kind) {
    var _a;
    ...
    const newVariable = new LocalVariable(identifier2.name, identifier2, init, context, kind);
...
var BlockScope = class extends ChildScope {
  ...
  addDeclaration(identifier2, context, init, kind) {
    ...
    if (kind === "var") {
      ...
      const declaredVariable = this.parent.addDeclaration(identifier2, context, init, kind);
      declaredVariable.markInitializersForDeoptimization();
      this.addHoistedVariable(name, declaredVariable);
      return declaredVariable;
    }
    return super.addDeclaration(identifier2, context, init, kind);
  }
};
...
var CatchBodyScope = class extends ChildScope {
  ...
  addDeclaration(identifier2, context, init, kind) {
    ...
    if (kind === "var") {
      ...
      const declaredVariable = this.parent.parent.addDeclaration(identifier2, context, init, kind);
      declaredVariable.markInitializersForDeoptimization();
      this.addHoistedVariable(name, declaredVariable);
      return declaredVariable;
    }
...
var UnaryExpression = class extends NodeBase {
  ...
  include(context, includeChildrenRecursively, _options) {
    if (!this.deoptimized)
      this.applyDeoptimizations();
    this.included = true;
    if (typeof this.getRenderedLiteralValue(includeChildrenRecursively) === "symbol" || this.argument.shouldBeIncluded(context)) {
      this.argument.include(context, includeChildrenRecursively);
      this.renderedLiteralValue = UnknownValue;
    }
  }
  ...
  function classBody(node, position, buffer) {
    ...
    if (bodyPosition) {
      ...
      for (let index = 0; index < length; index++) {
        ...
        body[index] = convertNode2(node, (buffer[nodePosition + 3] & 1) === 0 ? scope.instanceScope : scope, nodePosition, buffer);
      }
    } else {
      node.body = [];
    }
...
`);
  ...
  getDynamicImportStringAndAttributes(resolution, fileName) {
    ...
    return [
      resolution || "",
      ["es", "cjs"].includes(this.outputOptions.format) && this.outputOptions.externalImportAttributes || null
    ];
  }
...
var { bold, cyan, dim, gray, green, red, underline, yellow } = pc.createColors(import_node_process.env.FORCE_COLOR !== "0" && !import_node_process.env.NO_COLOR);
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-D2XKK3OY.js");
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-ZIOI4QBA.js.map
